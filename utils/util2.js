...
const useTime = begin => {
  ...
  let afterMin = parseInt(useTime - day * 24 * 60 * 60 - hour * 60 * 60 - min * 60);//取得算出分后剩余的秒数
  let time = null
  if (day == 0) {
    if (hour == 0) {
      if (min == 0) {
    ...
    time = `${day}天${hour}时${min}分${afterMin}秒`
  }
...
const initialTimePeriod = (tomorrow = false) => {
  ...
  if (limit_hour > 19) {
    ...
    start_time = start_hour < 10 ? '0' + start_hour + ':00' : start_hour + ':00'
  }
  let end_hour = 22;
  let time_list = [];
  while (start_hour < end_hour) {
    ...
    start_hour++;
    if ((start_hour >= 8 && start_hour <= 12) || (start_hour >= 15 && start_hour <= 19)) {
...
const uploadImageOne = function (opt, successCallback, errorCallback) {
  ...
  var count = opt.count || 1, sizeType = opt.sizeType || ['compressed'], sourceType = opt.sourceType || ['album', 'camera'],
    is_load = opt.is_load || true, uploadUrl = opt.url || '', inputName = opt.name || 'img';
  wx.chooseImage({
